...
namespace minico 
{
	...
	public:
			...
			: _sockfd(sockfd), _pRef(new int(1)), _port(port), _ip(std::move(ip))
		{
			if (sockfd > 0)
			{
				setNonBolckSocket();		//首先设置为非阻塞
			}
		}
		//默认是非阻塞的TCP协议的socket
		//我们期望的是能在fork子进程前打开某个文件句柄时就指定好：“这个句柄我在fork子进程后执行exec时就关闭”。其实时有这样的方法的：即所谓 的 close-on-exec。
		//要我们在创建socket的时候加上SOCK_CLOEXEC标志，就能够达到我们要求的效果，在fork子进程中执行exec的时候，会清理掉父进程创建的socket
		Socket()
