...
namespace minico {
	...
	//配合std::atomic_int定义的二元信号量使用，为1表示资源可以使用，为0表示资源不可使用 这就是自旋锁的实现
	class Spinlock
	{
	public:
		Spinlock()
		...
		void lock()
		{
			...
			//比较并交换被封装的值(strong)与参数 expected所指定的值是否相等
			//相等,则用val替换原子对象的旧值
		...
		void unlock()
		{
			sem_.store(1);	//std::atomic::store 函数将类型为 T 的参数 val 复制给原子对象所封装的值
		}
